...
channel.bind('my-event', function(data) {
  ...
  if (data.tipe=="notif_panggilan"){
    if (data.ukm_id==window.localStorage.getItem("ukm_id") || window.localStorage.getItem("level")=="1"  ){
      myApp.addNotification({
        title: 'Penyewaan Baru',
        message: 'Terdapat Pemesanan Penyewaan!'
      });
...
var welcomescreen_slides = [
  {
    ...
    text: 'Selamat Datang di Aplikasi Rumah , Aplikasi yang dapat memberikan informasi mengenai Rumah sekitarmu'
  },
  {
    id: 'slide1',
    picture: '<i class="fa fa-map-marker fa-5x"></i>',
    text: 'Memudahkan Anda untuk Menemukan Rumah  di sekitar anda, membuat hidup menjadi lebih mudah .'
  },
  {
    id: 'slide2',
    picture: '<i class="fa fa-location-arrow fa-5x"></i>',
    text: 'Bantu kami untuk mengumpulkan Rumah sekitar anda, dan dapatkan poin dari setiap pengajuan Rumah yang anda ajukan  '
  },
  {
    id: 'slide3',
  ...
  var options = {
   ...
   function onSuccess(position) {
      ...
      var username = window.localStorage.getItem("username");
       myApp.prompt(' Silahkan isi Pesan Pemanggilan / Pesanan  kepada Rumah  ', 'Panggil dan Kirim pesan',
          function (value) {
            var pesan;
            if (value==""){
        ...
        success : function(r){
          ...
          if (data.success==false){
            ...
            // mainView.router.back();
             ...
             myApp.addNotification({
                  message: "Pesanan Berhasil Dikirim ke Rumah !",
                  buttonkey:  {
                      text: 'Tutup',
                      // color: 'lightgreen'
                    }
  ...
  // myApp.alert('Akses Detail');
   ...
   $$.ajax({
        ...
        success : function(r){
            ...
            $$("div data-page[rincian-umkm]").find(".navbar-inner .center").html("Informasi Rumah ("+data.nama+")");
            ...
            // $$(".photo-browser .navbar .center").html("Gambar Rumah");
           ...
           if (loker.length>0){
            $$.each(loker,function(i,v){
              ...
              var img = "" ;
                ...
                if (imageExists(server+"/images/loker/"+v.id+".jpg")){
                img = server+"/images/loker/"+v.id+".jpg";
               }else{
                img = "img/no_image.jpg";
               }
...
$$(document).on('page:init', '.page[data-page="add-umkm"]', function (e) {
  ...
  $$(document).on('click', '.current-map', function (e) {
    ...
    // $$('#form-add-ukm').on('submit', function (e) { //submit add umkm
      ...
      //                   message: { 
      //                     berita : "Terdapat permintaan pendataan Rumah baru"
      //                   }
      //                  });
      //                   myApp.hidePreloader();
      //                   mainView.router.back();
...
function tolakTerima(status,id,telepon,comp){
  $$.ajax({
    ...
    success : function(r){
        ...
        if (data.success){
            myApp.addNotification({
              message: "Rumah telah di konfirmasi  ",
              button: {
                    text: 'Tutup',
                    // color: 'lightgreen'
                  }
...
$$(document).on('click', '.pending-acc', function (e) {
  ...
  // myApp.confirm(' Menyetujui Permintaan Rumah ? ','Konfirmasi', function () {
...
$$(document).on('click', '.pending-reject', function (e) {
  ...
  e.preventDefault();
  var ukm_id = $$(this).attr("ukm-id");
    myApp.confirm(' Menolak Permintaan Rumah ? ','Konfirmasi', function () {
...
$$(document).on('page:init', '.page[data-page="topup"]', function (e) {
  // alert("123");
...
$$(document).on('click', '.btn-refresh-order', function (e) {
  ...
  if (tabActive == undefined) tabActive = "#tab-11"; 
  getListOrder(window.localStorage.getItem("username"), tabActive);
      ...
      success : function(r){
        ...
        $$("#order-pending").html("");
         ...
         $$.each(data,function(i,data){
          ...
          '<div class="item-inner">'+
          '<div class="item-title-row">'+
          '<div class="item-title">'+data.pemilik+'</div>'+
          '<div class="item-after">'+data.alamat+' </div>'+

          // '<div class="item-after">'+moment(data.jam, "YYYY-MM-DD h:mm:ss").fromNow()+' </div>'+
          '</div>'+
          // '<div class="item-subtitle"><br><br>'+data.alamat+'</div>'+
          // '<div class="item-text" style="color:red">'+data.alamat+'</div>'+
          '</div>'+
          '</a>'+
          '</div>'+
    ...
    // $$("#rt").append("<option value=''>Semua RT</option>");
        ...
        // initMap();
          ...
          if (data.model.length!=0){
            ...
            myApp.addNotification({
              message: "Tidak Menemukan Rumah yang sesuai dengan Kata Kunci   ",
              button: {
                    text: 'Tutup',
                    // color: 'lightgreen'
                  }
     ...
     $$.each(data,function(i,v){  
          ...
          if (v.tipe=="1" || v.tipe=="2" || v.tipe==""){ // JIKA PEDAGANG TETAP MAKA
            ...
            icon = server+"/icon/rumah-question.png";  
             desc = "<p style='color:red'>Kami tidak bisa memastikan keberadaan Rumah ini</p>"
          }
...
$$(document).on('click', '.btn-atm-dekat', function(e){
   mainView.router.load({
...
//             <i class="fa fa-home fa-3x"></i>
      // ukm-id="'+v.id+'"  href="tabs-swipeable.html?id='+v.id+'"
            ...
            '<div class="item-media">'+tag_img+'</div>'+
            '<div class="item-inner">'+
            '<div class="item-title-row">'+
            '<div class="item-title">'+v.nama+'</div>'+
            '<div class="item-after">'+v.distance+' KM </div>'+
            '</div>'+
            '<div class="item-subtitle color-red" >Minimal '+v.minimal+' '+v.satuan_minimal+' </div>'+
            '<div class="item-subtitle color-red" data-harga-sewa='+v.harga+' >Harga Rp.'+numberWithCommas(v.harga)+'</div>'+
            '<div class="item-subtitle color-red" >'+
        ...
        $$("#direction").append("<li>Data Tidak Ditemukan </li>");
  ...
  $$.ajax({
        ...
        success : function(r){
              ...
              $$.each(result_marker,function(i,v){
                ...
                // if (wind)
                 ...
                 if (v.tipe=="3"){
                        ...
                        // },20000);
                    // jika Rumah ada pada daftar favoritku maka
                    if(window.localStorage.getItem("td_favorite").indexOf(v.id)!= -1){ // value exist                  
                         ...
                         if (list_td_favoritku.indexOf(v.id)!= -1){ // jika sudah di notifikasi maka
                          ...
                          else{ // memberi tahukan bahawa ada Rumah favorite
   
                            list_td_favoritku.push(v.id);
                            // myApp.addNotification({
                            //     message: "Terdapat Rumah Favorit kamu ("+v.nama+") dalam 1 KM",
                            //     buttonkey:  {
                            //         text: 'Tutup',
                            //         // color: 'lightgreen'
                            //     },
           ...
           // }
            ...
            // else{
            //    myApp.addNotification({
            //       message: "Tidak menemukan seluruh jenis Rumah   dalam 1 KM",
            //       button: {
            //         text: 'Tutup',
            //         // color: 'lightgreen'
            //       },
        ...
        success : function(r){
            ...
            if (data.model.length!=0){
              ...
              // myApp.toast('Tidak menemukan', '', {}).show(true);
               ...
               myApp.addNotification({
                  message: "Tidak menemukan  Rumah Sewaan  dalam jarak 1 KM",
                  button: {
                    text: 'Tutup',
                    // color: 'lightgreen'
                  },
...
$$('#form-register').on('form:success', function (e) {
  ...
  // alert('123');
      ...
      //         var message = "Rumah 2017 , Silahkan Verifikasi dengan kode "+data.number+"  ";
  ...
  function cekLevel(){
    ...
    // alert     
     if (lev=="3" || lev=="2"  || lev=="1"  || lev=="4"  || lev=="5"){ //jika penjual kliling
        if (!isNaN(window.localStorage.getItem("ukm_id")) || window.localStorage.getItem("level")=="1"  ){   
  ...
  $$("#li_button_ukmku").on("click",function(e){
    ...
    if ( !isNaN(window.localStorage.getItem("ukm_id") )  ){ 
      ...
      // myApp.alert("Akun ini belum memiliki Rumah ","Ups")
    }
  });
  ...
  // });
   ...
   // $$('.open-left-panel').on('click', function (e) {
    ...
    $$(document).on("click",".btn-add-status",function(){
     ...
     $$(document).on('page:init', '.page[data-page="request-list-atm"]', function (e) {
       myApp.closePanel("right");
      ...
      success : function(r){
          ...
          var data = JSON.parse(r);
            ...
            if (data.payload.data.length>0){
                $$("#ul-list-atm").html(" ");
                  // roundToTwo
                  $$.each(data.payload.data,function(i,v){
                    img = "img/no_image.jpg";
                   // }
                     // console
                     var dist = getDistance($("#val-lat").val(),$("#val-lon").val(),v.latitude,v.longitude,"K");
                    dist = roundToTwo(dist);
                   string = '<li class="hold-hapus-atm" data-lat='+v.latitude+' data-lng='+v.longitude+'  >'+
                    '<a href="#" class="item-link item-content">'+
                    '<div class="item-media"><img src="'+img+'" width="80"></div>'+
                    '<div class="item-inner">'+
                    '<div class="item-title-row" style="background-image:url()">'+
                    '<div class="item-title"> '+v.nama+'</div>'+
                    '<div class="item-after">'+dist+' KM </div>'+
                    '</div>'+
                    '<div class="item-text ">'+v.alamat+
                     '</div>'+
                    '</div>'+
                    '</a>'+
                    '</li>';
                    $$("#ul-list-atm").append(string);
                    // alert(string);
                  });
